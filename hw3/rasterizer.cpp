...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for(int x = x_min_bounding; x < x_max_bounding; x++) {
        for(int y = y_min_bounding; y < y_max_bounding; y++) {
            ...
            if(insideTriangle(x + 0.5, y + 0.5, t.v) && zp < depth_buf[get_index(x, y)]) {
                ...
                Eigen::Vector2f interpolated_texcoords = (alpha*t.tex_coords[0]/v[0].w() + beta*t.tex_coords[1]/v[1].w() + gamma*t.tex_coords[2]/v[2].w()) * Z;
